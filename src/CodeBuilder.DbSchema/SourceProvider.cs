...
namespace CodeBuilder.DbSchema
{
    ...
    public class SourceProvider : ISourceProvider
    {
        ...
        public string Name => "DbSchema";

        public Image Icon => Properties.Resources.dbschema;

        public string Description => "支持从 DbSchema 设计的 dbs 文件中导入数据表结构。";

        public void Initialize(IDevHosting hosting)
        {
            _hosting = hosting;
            _historyStorage = new HistoryStorage<string>(Path.Combine(_hosting.WorkPath, "history.dbschema"), 10);
        }
        ...
        public async Task<List<Table>> PreviewAsync(SourceOption option, CancellationToken cancellationToken = default)
        {
            ...
            {
                dialog.Filter = "DbSchema Project File|*.dbs";

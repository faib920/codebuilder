    ...
    internal class TemplateHelper
    {
        ...
        internal static byte[] ZipPackage(IDevHosting hosting, TemplateDefinition definition)
        {
            ...
            using (var archive = new ZipOutputStream(memory))
            {
                ...
                {
                    var publicEntryName = "template." + pathData[0] + "/public/";
                    archive.PutNextEntry(new ZipEntry(publicEntryName));
                    var publicRoot = Path.Combine(hosting.WorkPath, "templates", pathData[0], "public");
                    PutPublicScript(hosting, publicEntryName, publicRoot, definition.Public, archive);
                }
        ...
        /// <param name="hosting"></param>
        /// <returns></returns>
        internal static List<string> GetTemplateCategories(IDevHosting hosting)
        {
            return hosting.TemplateProvider.GetTemplates().Where(s => !string.IsNullOrEmpty(s.Category)).Select(s => s.Category).Distinct().ToList();
        }
        ...
        /// <param name="hosting"></param>
        /// <returns></returns>
        internal static List<string> GetTemplateLanguages(IDevHosting hosting)
        {
            return hosting.TemplateProvider.GetTemplates().Where(s => !string.IsNullOrEmpty(s.Language)).Select(s => s.Language).Distinct().ToList();
        }
        ...
        private static void PutPublicScript(IDevHosting hosting, string entryName, string root, List<string> files, ZipOutputStream archive)
        {
            foreach (var file in files)
            {
                ...
                if (File.Exists(filePath))
                {
                    PutFile(archive, entryName + file, filePath);
                }
            }
        }

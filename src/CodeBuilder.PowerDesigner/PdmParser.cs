    ...
    public class PdmParser
    {
        ...
        public Table ParseTable(IServiceProvider serviceProvider, PdmTable table)
        {
            ...
            foreach (XmlNode child in ndColumns)
            {
                ...
                if (dtNode != null)
                {
                    ...
                    column.DbType = DBMSManager.GetDbType(_databaseType, column.DataType);
        ...
        private XmlNode FindShortcutNode(XmlNode sNode)
        {
            ...
            for (var i = 1; i < sPath.Length; i++)
            {
                ...
                {
                    if (child.SelectSingleNode("a:Name", _nms)?.InnerText == sPath[i])
                    {
                        if (i == sPath.Length - 1)
                        {
                            fNode = child;
        ...
        private void ParseDbTypeAndPrecision(string dbType, Column column)
        {
            ...
            if (matches.Count > 0)
            {
                column.ColumnType = matches[0].Groups[0].Value;
                column.DataType = matches[0].Groups[1].Value;
                column.Length = matches[0].Groups[2].Value.To<long?>();
            }
            else
            {
                ...
                if (matches.Count > 0)
                {
                    column.ColumnType = matches[0].Groups[0].Value;
                    column.DataType = matches[0].Groups[1].Value;
                    column.Precision = matches[0].Groups[2].Value.To<int?>();
                    column.Scale = matches[0].Groups[3].Value.To<int?>();
                }
                else
                {

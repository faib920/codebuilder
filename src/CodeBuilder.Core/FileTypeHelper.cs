...
namespace CodeBuilder.Core
{
    ...
    public class FileTypeHelper
    {
        ...
        {
            _filters.Add(".dss", "数据源架构文件|*.dss");
            _filters.Add(".dso", "数据源开放文件|*.dso");
            _filters.Add(".dsr", "数据源关系文件|*.dsr");
            _filters.Add(".template", "模板定义文件|*.template");
        ...
        public static int GetFilterIndex(string extension)
        {
            ...
            {
                if (kvp.Key.Equals(extension, System.StringComparison.OrdinalIgnoreCase))
                {
                    return index;
                }
        ...
        {
            if (_filters.TryGetValue(extension, out var filter))
            {
                return filter;
            }
